...
def collate_fn(batch):
    ...
    coord, feat, label, pid = list(zip(*batch))
    ...
    return torch.cat(coord), torch.cat(feat), torch.cat(label), torch.IntTensor(offset), torch.IntTensor(pid)


def collate_fn_submit(batch):
    # coord, feat, label = list(zip(*batch))
    coord, feat, pid = list(zip(*batch))
    ...
    # return torch.cat(coord), torch.cat(feat), torch.cat(label), torch.IntTensor(offset)
    return torch.cat(coord), torch.cat(feat), torch.IntTensor(offset), torch.IntTensor(pid)
...
def data_prepare(coord, feat, label, split='train', transform=None, shuffle_index=False):
    if split == 'train' and transform:
        coord, label = transform(coord, label)

    if split == 'train' and shuffle_index:
        shuf_idx = np.arange(coord.shape[0])
        np.random.shuffle(shuf_idx)
        coord, feat, label = coord[shuf_idx], feat[shuf_idx], label[shuf_idx]
