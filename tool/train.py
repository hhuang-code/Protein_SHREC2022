...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    # add weights with inverse propotional to the number of vertices per class
    f = open(os.path.join(args.data_root, 'train_statistics.json'), 'r')
    stats = json.loads(f.read())
    if args.classes == 2:
        weight = torch.Tensor([val for key, val in stats.items()])
        weight = torch.Tensor([weight[0].item(), weight[1:].sum().item()])
        weight = weight.sum() / weight
    else:
        weight = torch.Tensor([val for key, val in stats.items()])
        weight = weight.sum() / weight

    criterion = nn.CrossEntropyLoss(weight=weight, ignore_index=args.ignore_label)
    ...
    train_data = SHREC2022(data_root=args.data_root, split='train', transform=train_transform, shuffle_index=True,
                           curvatureM=args.curvatureM, curvatureG=args.curvatureG, curvatureMAX=args.curvatureMAX,
                           curvatureMIN=args.curvatureMIN)
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (coord, feat, target, offset, pid) in enumerate(train_loader):  # (n, 3), (n, 3), (n,), (b,), (b,)
        data_time.update(time.time() - end)

        # coord, feat, target, offset = coord.cuda(non_blocking=True), feat.cuda(non_blocking=True), \
        ...
        # non_site_idx, site_idx = torch.nonzero(target == 0).squeeze(-1), torch.nonzero(target != 0).squeeze(-1)
        # perm = torch.randperm(non_site_idx.shape[0])
        # partial_non_site_idx = non_site_idx[perm[:(site_idx.shape[0] // 2)]]
        # idx = torch.cat([site_idx, partial_non_site_idx], dim=0)
        #
        # perm = torch.randperm(idx.shape[0])
        # idx = idx[perm]

        # loss = criterion.cuda()(output, torch.zeros_like(output).scatter_(1, target.unsqueeze(-1), 1))
        loss = criterion.cuda()(output, target)
